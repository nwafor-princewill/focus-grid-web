...
const FAQContactCTA: React.FC = () => {
  return (
    <section className="relative w-full bg-white flex justify-center px-4 mb-[160px] overflow-visible">
      ...
      >
        ...
        {/* --- DYNAMIC BACKGROUND QUESTION MARKS (Enhanced Visibility & More Sizes) --- */}
        {/* Large - Bottom Left */}
        <div className="absolute -left-6 -bottom-8 opacity-[0.12] pointer-events-none animate-float-gentle">
            <img src={questionImg} alt="" className="w-[180px] h-auto grayscale" />
        </div>
        {/* Large - Top Right */}
        <div className="absolute -right-10 -top-10 opacity-[0.1] pointer-events-none animate-drift-slow">
            <img src={questionImg} alt="" className="w-[160px] h-auto grayscale rotate-[45deg]" />
        </div>
        {/* Medium - Top Left */}
        <div className="absolute left-[15%] top-4 opacity-[0.08] pointer-events-none animate-drift-slow" style={{ animationDelay: '1.5s' }}>
            <img src={questionImg} alt="" className="w-[90px] h-auto grayscale rotate-[-20deg]" />
        </div>
        {/* Medium - Middle Right */}
        <div className="absolute right-[12%] top-[30%] opacity-[0.1] pointer-events-none animate-float-gentle" style={{ animationDelay: '3s' }}>
            <img src={questionImg} alt="" className="w-[70px] h-auto grayscale" />
        </div>
        {/* Small - Bottom Center-ish */}
        <div className="absolute left-[40%] bottom-4 opacity-[0.15] pointer-events-none animate-star">
            <img src={questionImg} alt="" className="w-[45px] h-auto grayscale" />
        </div>
        {/* Tiny Scatter */}
        <div className="absolute right-[30%] top-10 opacity-[0.1] animate-float-gentle">
            <img src={questionImg} alt="" className="w-[30px] h-auto grayscale" />
        </div>
        ...
        <div className="flex justify-center items-center relative z-10">
          ...
          <img 
            src={questionImg} 
            alt="Question" 
            className="w-[70.3px] h-[70.3px] object-contain transition-all duration-700 group-hover:scale-110 group-hover:rotate-[360deg]"
            style={{ 
                opacity: 0.45, 
        ...
        <div className="flex flex-col items-center gap-2 text-center px-4 relative z-10">
          ...
          <p 
            className="text-[16px] font-light leading-[140%] text-[#1E1E1E] opacity-70 max-w-[600px]"
            style={{ fontFamily: 'Funnel Display, sans-serif' }}
          >
        ...
        <Link to="/contact" className="relative z-10">
          <button 
            className="w-[210px] h-[52px] bg-[#E6F6EE] rounded-[100px] px-[30px] py-[10px] 
                       flex items-center justify-center gap-3 transition-all duration-300 
                       hover:bg-[#00A550] hover:shadow-lg hover:scale-105 group/btn active:scale-95"
          >
            <span 
              className="text-[14px] font-semibold text-[#00A550] group-hover/btn:text-white uppercase tracking-wider transition-colors duration-300"
              style={{ fontFamily: 'Funnel Display, sans-serif' }}
            >
              CONTACT US
            </span>
            <img 
              src={northEastIcon} 
              alt="" 
              className="w-[18px] h-[18px] transition-all duration-300 group-hover/btn:translate-x-1 group-hover/btn:-translate-y-1 group-hover/btn:brightness-0 group-hover/btn:invert"
            />
          </button>
        </Link>
      </div>
