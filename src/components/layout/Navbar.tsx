      ...
      <div className="max-w-[1240px] mx-auto px-4 relative mt-8 lg:mt-0">
        <div className={`flex items-center justify-between px-5 py-5 transition-all duration-500 rounded-[1000px] 
          ...
          <Link to="/contact" className="hidden lg:block">
            <button className="h-[40px] bg-[#00A550] rounded-[100px] px-[30px] py-[10px] flex items-center gap-2 transition-all duration-300 hover:bg-[#008f44] hover:scale-[1.02] hover:shadow-[0_4px_12px_rgba(0,165,80,0.15)] group">
              <span className="text-sm font-medium text-white" style={{ fontFamily: 'Funnel Display, sans-serif' }}>CONTACT US</span>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="transition-transform duration-300 group-hover:translate-x-1">
      ...
      <div className={`fixed inset-0 z-50 lg:hidden transition-all duration-500 ${mobileMenuOpen ? 'visible opacity-100' : 'invisible opacity-0'}`}>
        ...
        <div className="relative h-full flex flex-col justify-center px-8 gap-8">
          {navLinks.map((link, index) => (
            <Link key={link.path} to={link.path} onClick={() => setMobileMenuOpen(false)} className={`text-4xl font-semibold transition-all duration-700 ${isActive(link.path) ? 'text-[#00A550]' : 'text-[#545454]'} ${mobileMenuOpen ? 'translate-x-0 opacity-100' : '-translate-x-12 opacity-0'}`} style={{ fontFamily: 'Funnel Display, sans-serif', transitionDelay: `${index * 100}ms` }}>
              {link.label}
            </Link>
          ))}
          <Link to="/contact" onClick={() => setMobileMenuOpen(false)} className={`mt-6 transition-all duration-700 delay-[400ms] ${mobileMenuOpen ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'}`}>
